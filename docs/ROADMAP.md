...
`lib/googleWorkspace.ts` is the GoogleSyncService, `lib/scheduler.ts` is the PlanningEngine, `lib/dailySummary.ts` is the DailySummaryGenerator, `lib/notificationService.ts` is the NotificationService, `lib/backgroundSyncService.ts` is the BackgroundSyncService, and `lib/reminderEngine.ts` (core logic in `lib/reminderPlan.ts`) is the ReminderEngine.
