...
const versionCode = Application.nativeBuildVersion
  ?? (Constants.expoConfig?.android?.versionCode != null ? String(Constants.expoConfig.android.versionCode) : 'unknown');

export function getBuildInfo() {
  return { appVersion, versionCode, releaseTag: FLOWOS_BUILD.releaseTag, commitSha: FLOWOS_BUILD.commitSha };
}

export function BuildInfo({ inline = false }: { inline?: boolean }) {
  const info = getBuildInfo();
  return <View pointerEvents="none" style={inline ? styles.inlineContainer : styles.container}>
    <Text selectable style={styles.text}>FlowOS {info.appVersion} · versionCode {info.versionCode}</Text>
  </View>;
}
