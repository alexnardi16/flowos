...
export function ManageSheet({ item, onClose }: { item: Commitment; onClose: () => void }) {
  ...
  function handleDatePicked(selected:Date){const value=`${selected.getFullYear()}-${String(selected.getMonth()+1).padStart(2,'0')}-${String(selected.getDate()).padStart(2,'0')}`;if(datePickerTarget==='start')updateStartDate(value);else if(datePickerTarget==='end')updateEndDate(value);setDatePickerTarget(null);}
  async function save() {
    setBusy(true);
    try {
      ...
      const updated: Commitment = {
        ...
        kind,
        priority: kind === 'task' && priority.trim() ? Math.max(1, Number(priority)) : undefined,
        allDay,
        durationMinutes,
  ...
  return <Modal visible animationType="slide" onRequestClose={onClose}>
    <ScrollView contentContainerStyle={styles.wrap}>
      ...
      <View style={styles.row}><TextInput value={customDays} onChangeText={v=>setCustomDays(v.replace(/\D/g,''))} keyboardType="number-pad" placeholder="giorni\nprima" style={[styles.input,{flex:1}]}/><TextInput value={customHours} onChangeText={v=>setCustomHours(v.replace(/\D/g,''))} keyboardType="number-pad" placeholder="ore\nprima" style={[styles.input,{flex:1}]}/><TextInput value={customMinutes} onChangeText={v=>setCustomMinutes(v.replace(/\D/g,''))} keyboardType="number-pad" placeholder="minuti\nprima" style={[styles.input,{flex:1}]}/><Pressable onPress={()=>{const m=(Number(customDays)||0)*1440+(Number(customHours)||0)*60+(Number(customMinutes)||0);if(m>0){addReminder(m);setCustomDays('');setCustomHours('');setCustomMinutes('');}}} style={styles.presetButton}><Text style={styles.presetButtonText}>Aggiungi</Text></Pressable></View>
