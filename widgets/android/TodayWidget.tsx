...
function visibleCount(heightDp = 180) { if (heightDp < 190) return 2; if (heightDp < 260) return 3; if (heightDp < 340) return 4; if (heightDp < 430) return 6; return 8; }
function compact(text:string,max=34){return text.length<=max?text:`${text.slice(0,max-1)}…`;}
export function TodayWidget({ items, heightDp }: AndroidTodayWidgetProps) {
  const visible = items.slice(0, visibleCount(heightDp));
  return <FlexWidget style={{ width:'match_parent', height:'match_parent', padding:10, backgroundColor:BG, borderRadius:20, flexDirection:'column' }} clickAction="OPEN_URI" clickActionData={{ uri:'flowos://today' }} accessibilityLabel="FlowOS: attività di oggi">
    <FlexWidget style={{ width:'match_parent', flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingBottom:5 }}>
      ...
      <FlexWidget style={{ flexDirection:'row', alignItems:'center' }}>
        <FlexWidget style={{ width:30, height:26, marginRight:4, borderRadius:13, backgroundColor:'#DDE2FF', justifyContent:'center', alignItems:'center' }} clickAction="OPEN_URI" clickActionData={{ uri:'flowos://today?widgetAction=sync' }} accessibilityLabel="Sincronizza FlowOS e Google"><TextWidget text="↻" style={{ fontSize:16, fontWeight:'bold', color:INK }}/></FlexWidget>
    ...
    <ListWidget style={{ width:'match_parent', height:'match_parent', backgroundColor:BG }}>
      {visible.length ? visible.map((item)=><FlexWidget key={item.id} style={{ width:'match_parent', height:54, marginVertical:2, paddingHorizontal:8, paddingVertical:5, borderRadius:12, backgroundColor:'#FFFFFF', flexDirection:'row', alignItems:'center' }}>
        <FlexWidget style={{ width:5, height:34, marginRight:7, borderRadius:3, backgroundColor:item.kind==='Evento'?'#6C7BE8':item.kind==='Task'?'#E5A73B':'#45B887' }}/>
