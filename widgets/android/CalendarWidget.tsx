...
function estimatedLines(text:string,charsPerLine=9){return Math.max(1,Math.ceil(text.length/charsPerLine));}
function itemHeight(item:AndroidCalendarItem){return Math.max(14,estimatedLines(`${item.time} ${item.title}`)*9+3);}
function dayHeight(day:AndroidCalendarDay){return Math.max(82,34+day.items.reduce((sum,item)=>sum+itemHeight(item),0));}
      ...
      {weeks.map(week=>{const height=Math.max(...week.days.map(dayHeight));return <FlexWidget key={`${week.title}-${week.days[0]?.dateKey}`} style={{ width:'match_parent',flexDirection:'column',marginVertical:2 }}>
        ...
        <FlexWidget style={{ width:'match_parent',flexDirection:'row' }}>
          {week.days.slice(0,7).map(day=><FlexWidget key={day.dateKey} style={{ flex:1,height,marginHorizontal:1,padding:3,borderRadius:8,backgroundColor:day.isToday?'#E8ECFF':'#FFFFFF',flexDirection:'column' }}>
            ...
            {day.items.map(item=><TextWidget key={item.id} text={`${item.time} ${item.title}`} style={{ fontSize:7,color:INK,marginTop:2 }}/>) }
