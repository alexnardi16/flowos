...
const BG='#F1F4FF'; const INK='#172033'; const MUTED='#697386'; const PRIMARY='#4254C5'; const DAY_WIDTH=14.2857;
function estimatedLines(text:string,charsPerLine=22){return Math.max(1,Math.ceil(text.length/charsPerLine));}
function itemHeight(item:AndroidCalendarItem){return Math.max(16,estimatedLines(`${item.time ? `${item.time} ` : ''}${item.title}`)*10+4);}
    ...
    <ListWidget style={{ width:'match_parent',height:'match_parent',backgroundColor:BG }}>
      {weeks.map(week=>{const height=Math.max(...week.days.map(dayHeight));return <FlexWidget key={`${week.title}-${week.days[0]?.dateKey}`} style={{ width:'match_parent',flexDirection:'column',marginVertical:2 }}>
        ...
        <FlexWidget style={{ width:'match_parent',flexDirection:'row' }}>
          {week.days.slice(0,7).map(day=><FlexWidget key={day.dateKey} style={{ width:`${DAY_WIDTH}%`,height,marginHorizontal:1,padding:3,borderRadius:8,backgroundColor:day.isToday?'#E8ECFF':'#FFFFFF',flexDirection:'column',minWidth:0,alignSelf:'stretch' }}>
            <TextWidget text={day.label} style={{ fontSize:8,fontWeight:'bold',color:day.isToday?PRIMARY:MUTED }}/>
            {day.items.map(item=><TextWidget key={item.id} text={`${item.time ? `${item.time} ` : ''}${item.title}`} style={{ fontSize:8,color:INK,marginTop:2 }}/>) }
