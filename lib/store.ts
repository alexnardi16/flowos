...
export const useFlowStore=create<State>()(persist((set,get)=>{
  const pushGoogleAndRefresh=async()=>{try{await pushPendingToGoogle();const remote=await loadCommitments();set({commitments:remote});refreshWidget(remote);refreshNotifications(remote);refreshNotifications(remote);refreshNotifications(remote);}catch(error){void logNotificationEvent('auto-push-failed',error,'warn');}};
  return ({
  commitments:[],syncing:false,
  addCommitment:async commitment=>{set(state=>({commitments:[commitment,...state.commitments]}));refreshWidget(get().commitments);await saveCommitment(commitment);refreshNotifications(get().commitments);},
  ...
  complete:async id=>{const item=get().commitments.find(c=>c.id===id);if(!item)return;const updated={...item,status:'done' as const};const next=materializeNextOccurrence(updated);const nextCommitments=next?[next,...get().commitments.map(c=>c.id===id?updated:c)]:get().commitments.map(c=>c.id===id?updated:c);set({commitments:nextCommitments});refreshWidget(nextCommitments);await saveCommitment(updated);if(next)await saveCommitment(next);await pushGoogleAndRefresh();refreshNotifications(get().commitments);showSnackbar('Attività completata','Annulla',()=>{void(async()=>{const restored=next?get().commitments.filter(c=>c.id!==next.id).map(c=>c.id===id?item:c):get().commitments.map(c=>c.id===id?item:c);set({commitments:restored});await saveCommitment(item);if(next)await removeCommitmentOnlyFromFlowOS(next.id);await pushGoogleAndRefresh();refreshWidget(restored);})();});},
  postpone:async id=>{const item=get().commitments.find(c=>c.id===id);if(!item)return;const base=item.scheduledAt??item.dueAt??new Date().toISOString();const nextDay=new Date(new Date(base).getTime()+86400000).toISOString();const updated={...item,status:item.kind==='event'?'scheduled':item.status,scheduledAt:item.scheduledAt?nextDay:undefined,dueAt:item.dueAt?nextDay:undefined} as Commitment;const nextCommitments=get().commitments.map(c=>c.id===id?updated:c);set({commitments:nextCommitments});refreshWidget(nextCommitments);await saveCommitment(updated);await pushGoogleAndRefresh();refreshNotifications(get().commitments);showSnackbar('Attività rimandata di 1 giorno','Annulla',()=>{void(async()=>{const restored=get().commitments.map(c=>c.id===id?item:c);set({commitments:restored});await saveCommitment(item);await pushGoogleAndRefresh();refreshWidget(restored);})();});},
  updateCommitment:async updated=>{const previous=get().commitments.find(item=>item.id===updated.id);const nextCommitments=get().commitments.map(item=>item.id===updated.id?updated:item);set({commitments:nextCommitments});refreshWidget(nextCommitments);await saveCommitment(updated);await pushGoogleAndRefresh();refreshNotifications(get().commitments);if(previous)showSnackbar('Modifiche salvate','Annulla',()=>{void(async()=>{const restored=get().commitments.map(c=>c.id===updated.id?previous:c);set({commitments:restored});await saveCommitment(previous);await pushGoogleAndRefresh();refreshWidget(restored);})();});},
  removeOnlyFromFlowOS:async id=>{const item=get().commitments.find(c=>c.id===id);await removeCommitmentOnlyFromFlowOS(id);const next=get().commitments.filter(c=>c.id!==id);set({commitments:next});refreshWidget(next);refreshNotifications(next);if(item)showSnackbar('Attività eliminata da FlowOS','Annulla',()=>{void(async()=>{const restored=[item,...get().commitments];set({commitments:restored});await saveCommitment({...item,deletedAt:undefined});refreshWidget(restored);})();});},
