...
export const useFlowStore=create<State>()(persist((set,get)=>{
  ...
  addCommitment:async commitment=>{
    const normalized = commitment.kind === 'task' && commitment.status !== 'done'
      ? { ...commitment, priority: commitment.priority ?? nextTaskPriority(get().commitments) }
      : commitment;
  ...
  complete:async id=>{const item=get().commitments.find(c=>c.id===id);if(!item)return;const updated={...item,status:'done' as const};const next=materializeNextOccurrence(updated);const rawNext=next?[next,...get().commitments.map(c=>c.id===id?updated:c)]:get().commitments.map(c=>c.id===id?updated:c);const nextCommitments=normalizeTaskPriorities(rawNext);set({commitments:nextCommitments});refreshWidget(nextCommitments);await saveCommitment(updated);if(next)await saveCommitment(next);for(const changed of nextCommitments.filter(c=>c.kind==='task'&&c.status!=='done')){const previous=get().commitments.find(c=>c.id===changed.id);if(previous?.priority!==changed.priority)await saveCommitment(changed);}await pushGoogleAndRefresh();showSnackbar('Attività completata','Annulla',()=>{void(async()=>{const restored=next?get().commitments.filter(c=>c.id!==next.id).map(c=>c.id===id?item:c):get().commitments.map(c=>c.id===id?item:c);set({commitments:restored});await saveCommitment(item);if(next)await removeCommitmentOnlyFromFlowOS(next.id);await pushGoogleAndRefresh();refreshWidget(restored);refreshNotifications(restored);})();});},
  ...
  updateCommitment:async updated=>{const previous=get().commitments.find(item=>item.id===updated.id);const base=upsertCommitmentState(get().commitments,updated.kind==='task'&&updated.status!=='done'?{...updated,priority:updated.priority??nextTaskPriority(get().commitments)}:updated);const nextCommitments=updated.kind==='task'&&updated.status!=='done'?reorderTaskPriorities(base,updated.id,updated.priority??nextTaskPriority(base)):normalizeTaskPriorities(base);set({commitments:nextCommitments});refreshWidget(nextCommitments);for(const changed of nextCommitments){const before=get().commitments.find(c=>c.id===changed.id);if(!before||before.priority!==changed.priority||changed.id===updated.id)await saveCommitment(changed);}await pushGoogleAndRefresh();refreshNotifications(get().commitments);if(previous)showSnackbar('Modifiche salvate','Annulla',()=>{void(async()=>{const restored=get().commitments.map(c=>c.id===updated.id?previous:c);set({commitments:restored});await saveCommitment(previous);await pushGoogleAndRefresh();refreshWidget(restored);})();});},
  removeOnlyFromFlowOS:async id=>{const item=get().commitments.find(c=>c.id===id);await removeCommitmentOnlyFromFlowOS(id);const next=normalizeTaskPriorities(removeCommitmentState(get().commitments,id));set({commitments:next});refreshWidget(next);refreshNotifications(next);if(item)showSnackbar('Attività eliminata da FlowOS','Annulla',()=>{void(async()=>{const restored=[item,...get().commitments];set({commitments:restored});await saveCommitment({...item,deletedAt:undefined});refreshWidget(restored);refreshNotifications(restored);})();});},
  removeAlsoFromGoogle:async id=>{const item=get().commitments.find(c=>c.id===id);if(!item)return;await deleteCommitmentAlsoFromGoogle(item);const next=normalizeTaskPriorities(get().commitments.filter(c=>c.id!==id));set({commitments:next});refreshWidget(next);refreshNotifications(next);showSnackbar('Eliminata da FlowOS e da Google');},
