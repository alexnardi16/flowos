...
export const useFlowStore=create<State>()(persist((set,get)=>({
  ...
  updateCommitment:async updated=>{const previous=get().commitments.find(item=>item.id===updated.id);const nextCommitments=get().commitments.map(item=>item.id===updated.id?updated:item);set({commitments:nextCommitments});refreshWidget(nextCommitments);await saveCommitment(updated);pushGoogleSafely();if(previous)showSnackbar('Modifiche salvate','Annulla',()=>{void(async()=>{const restored=get().commitments.map(c=>c.id===updated.id?previous:c);set({commitments:restored});await saveCommitment(previous);pushGoogleSafely();refreshWidget(restored);})();});},
