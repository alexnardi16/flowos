...
export const useFlowStore=create<State>()(persist((set,get)=>{
  const pushGoogleAndRefresh=async()=>{try{await flushOfflineQueue();await pushPendingToGoogle();const remote=await loadCommitments();const pendingIds=await getPendingCommitmentIds();const merged=mergeRemoteCommitments(remote,get().commitments,pendingIds);set({commitments:merged});refreshWidget(merged);refreshNotifications(merged);}catch(error){const pendingIds=await getPendingCommitmentIds();const preserved=mergeRemoteCommitments([],get().commitments,pendingIds);set({commitments:preserved});refreshWidget(preserved);refreshNotifications(preserved);void logNotificationEvent('auto-push-failed',error,'warn');}};
  return ({
  ...
  hydrateFromCloud:async()=>{set({syncing:true});try{await flushOfflineQueue();const pendingIds=await getPendingCommitmentIds();const remote=await loadCommitments();const merged=mergeRemoteCommitments(remote,get().commitments,pendingIds);set({commitments:merged});refreshWidget(merged);refreshNotifications(merged);}finally{set({syncing:false});}},
