...
export const useFlowStore=create<State>()(persist((set,get)=>{
  ...
  hydrateFromCloud:async()=>{set({syncing:true});try{await flushOfflineQueue();const remote=await loadCommitments();set({commitments:remote});refreshWidget(remote);refreshNotifications(remote);}finally{set({syncing:false});}},
  complete:async id=>{const item=get().commitments.find(c=>c.id===id);if(!item)return;const updated={...item,status:'done' as const};const next=materializeNextOccurrence(updated);const nextCommitments=next?[next,...get().commitments.map(c=>c.id===id?updated:c)]:get().commitments.map(c=>c.id===id?updated:c);set({commitments:nextCommitments});refreshWidget(nextCommitments);await saveCommitment(updated);if(next)await saveCommitment(next);await pushGoogleAndRefresh();refreshNotifications(get().commitments);showSnackbar('Attività completata','Annulla',()=>{void(async()=>{const restored=next?get().commitments.filter(c=>c.id!==next.id).map(c=>c.id===id?item:c):get().commitments.map(c=>c.id===id?item:c);set({commitments:restored});await saveCommitment(item);if(next)await removeCommitmentOnlyFromFlowOS(next.id);await pushGoogleAndRefresh();refreshWidget(restored);refreshNotifications(restored);})();});},
