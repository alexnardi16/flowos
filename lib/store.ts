...
export const useFlowStore=create<State>()(persist((set,get)=>{
  ...
  addCommitment:async commitment=>{
    const normalized = commitment.kind === 'task' && commitment.status !== 'done'
      ? { ...commitment, priority: commitment.priority ?? nextTaskPriority(get().commitments, commitment) }
      : commitment;
  ...
  rolloverTodayTasks:async()=>{if(taskRolloverInFlight)return taskRolloverInFlight;taskRolloverInFlight=(async()=>{const current=get().commitments;const result=rolloverIncompleteTasks(current,new Date());if(!result.changed.length)return;set({commitments:result.commitments});refreshWidget(result.commitments);refreshNotifications(result.commitments);for(const item of result.changed)await saveCommitment(item);try{await pushGoogleAndRefresh();}catch(error){void logNotificationEvent('task-rollover-push-failed',error,'warn');}})().finally(()=>{taskRolloverInFlight=null;});return taskRolloverInFlight;},
  ...
  postpone:async id=>{const item=get().commitments.find(c=>c.id===id);if(!item)return;const base=item.scheduledAt??item.dueAt??new Date().toISOString();const nextDay=new Date(new Date(base).getTime()+86400000).toISOString();const updated={...item,status:item.kind==='event'?'scheduled':item.status,scheduledAt:item.scheduledAt?nextDay:undefined,dueAt:item.dueAt?nextDay:undefined} as Commitment;const nextCommitments=normalizeTaskPriorities(get().commitments.map(c=>c.id===id?updated:c));set({commitments:nextCommitments});refreshWidget(nextCommitments);for(const changed of nextCommitments){const before=get().commitments.find(c=>c.id===changed.id);if(!before||before.priority!==changed.priority||changed.id===id)await saveCommitment(changed);}await pushGoogleAndRefresh();refreshNotifications(get().commitments);showSnackbar('Attività rimandata di 1 giorno','Annulla',()=>{void(async()=>{const restored=get().commitments.map(c=>c.id===id?item:c);set({commitments:restored});await saveCommitment(item);await pushGoogleAndRefresh();refreshWidget(restored);refreshNotifications(restored);})();});},
  updateCommitment:async updated=>{const current=get().commitments;const previous=current.find(item=>item.id===updated.id);const base=upsertCommitmentState(current,updated.kind==='task'&&updated.status!=='done'?{...updated,priority:updated.priority??nextTaskPriority(current,updated)}:updated);const normalizedBase=normalizeTaskPriorities(base);
const nextCommitments=updated.kind==='task'&&updated.status!=='done'?reorderTaskPriorities(normalizedBase,updated.id,updated.priority??nextTaskPriority(normalizedBase,updated)):normalizedBase;set({commitments:nextCommitments});refreshWidget(nextCommitments);for(const changed of nextCommitments){const before=current.find(c=>c.id===changed.id);if(!before||before.priority!==changed.priority||changed.id===updated.id)await saveCommitment(changed);}await pushGoogleAndRefresh();refreshNotifications(get().commitments);if(previous)showSnackbar('Modifiche salvate','Annulla',()=>{void(async()=>{const restored=get().commitments.map(c=>c.id===updated.id?previous:c);set({commitments:restored});await saveCommitment(previous);await pushGoogleAndRefresh();refreshWidget(restored);})();});},
