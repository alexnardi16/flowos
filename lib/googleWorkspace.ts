...
const GUARD_FUNCTION_URL = `${SUPABASE_URL}/functions/v1/google-sync-guard`;
const REQUEST_TIMEOUT_MS = 60_000;
const STALE_SYNC_THRESHOLD_MS = 3 * 60 * 1000;

function currentRange(): GoogleSyncRange {
...
async function updateSyncFailure(message: string) {
  ...
  if (!data.user) return;
  const { error } = await supabase.from('google_connections').update({ last_sync_status: 'error', last_sync_error: message }).eq('user_id', data.user.id);
...
export function isSyncGenuinelyStale(connection: GoogleWorkspaceStatus['connection'], now: Date = new Date()) {
  if (!connection || connection.last_sync_status !== 'syncing') return false;
  if (!connection.updated_at) return true;
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      const response = await fetch(FUNCTION_URL, {
        ...
        headers: { Authorization: `Bearer ${accessToken}`, apikey: SUPABASE_KEY, 'Content-Type': 'application/json', 'x-client-info': 'flowos-google-sync/3.5' },
        body: JSON.stringify(body),
      });
      const raw = await response.text();
      let payload: any = null;
      try { payload = raw ? JSON.parse(raw) : null; } catch { payload = raw ? { error: raw } : null; }
      ...
      const message = namedError?.name === 'AbortError' ? 'La richiesta di sincronizzazione ha superato il tempo massimo consentito.' : errorMessage(error);
...
async function invokeSyncGuard() {
  const { data: sessionData, error: sessionError } = await supabase.auth.getSession();
  if (sessionError) throw sessionError;
  const accessToken = sessionData.session?.access_token;
  if (!accessToken) throw new Error('Sessione FlowOS scaduta. Esci e accedi nuovamente.');
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
  recordDiagnostic('google-sync-guard-request-start');
  try {
    const response = await fetch(GUARD_FUNCTION_URL, {
      method: 'POST',
      signal: controller.signal,
      headers: { Authorization: `Bearer ${accessToken}`, apikey: SUPABASE_KEY, 'Content-Type': 'application/json', 'x-client-info': 'flowos-google-sync-guard/1.1' },
      body: JSON.stringify({ action: 'guard' }),
    });
    const raw = await response.text();
    let payload: any = null;
    try { payload = raw ? JSON.parse(raw) : null; } catch { payload = raw ? { error: raw } : null; }
    recordDiagnostic('google-sync-guard-response', { status: response.status, ok: response.ok, error: payload?.error ?? null });
    if (!response.ok || payload?.error) throw new Error(String(payload?.error || `Google Sync Guard ha risposto con stato ${response.status}.`));
    recordDiagnostic('google-sync-guard-completed', payload ?? {});
    return payload;
  } catch (error) {
    const namedError = error as { name?: string };
    const message = namedError?.name === 'AbortError' ? 'Il controllo di sicurezza Google ha superato il tempo massimo consentito.' : errorMessage(error);
    recordDiagnostic('google-sync-guard-request-failed', { message }, 'error');
    throw new Error(`Controllo sincronizzazione Google fallito: ${message}`);
  } finally {
    clearTimeout(timeout);
  }
}
...
export async function signInWithGoogle() {
  const redirectTo = Platform.OS === 'web' && typeof window !== 'undefined' ? `${window.location.origin}/today` : 'flowos://today';
  recordDiagnostic('google-oauth-started', { platform: Platform.OS, redirectTo });
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'google',
    options: { redirectTo, skipBrowserRedirect: Platform.OS !== 'web', scopes: GOOGLE_SCOPES, queryParams: { access_type: 'offline', include_granted_scopes: 'true' } },
  });
...
export async function connectGoogleFromSession(session: Session, force = false) {
  ...
  const result = await invoke({ action: 'connect', providerToken: session.provider_token, providerRefreshToken: session.provider_refresh_token, scopes: GOOGLE_SCOPES, expiresIn: 3600 });
...
let syncInFlight: Promise<{ pushed: number; events: number; tasks: number; range?: GoogleSyncRange }> | null = null;

export function subscribeToSyncProgress(onProgress: (progress: SyncProgress) => void) {
  activeProgressListeners.add(onProgress);
  return () => { activeProgressListeners.delete(onProgress); };
}

export function isGoogleSyncInFlight() { return syncInFlight !== null; }
...
export async function syncGoogleWorkspace(onProgress?: (progress: SyncProgress) => void) {
  ...
  if (syncInFlight) {
    ...
    try { return await syncInFlight; } finally { if (onProgress) activeProgressListeners.delete(onProgress); }
  }
  ...
  try { return await syncInFlight; } finally { syncInFlight = null; if (onProgress) activeProgressListeners.delete(onProgress); }
}
  ...
  try {
    ...
    for (const calendar of calendars) {
      for (const year of years) {
        ...
        do {
          const page = await invoke({ action: 'sync-calendar-page', calendarId: calendar.google_calendar_id, year, pageToken });
    ...
    try { await invoke({ action: 'sync-fail', message }, 0); } catch (markError) { recordDiagnostic('google-sync-server-mark-error-failed', markError, 'error'); }
    await updateSyncFailure(message);
...
export async function setSyncRange(startDate: string | null, endDate: string | null) { return invoke({ action: 'set-sync-range', startDate, endDate }); }
export function friendlyCalendarName(name: string, ownEmail?: string | null) { return ownEmail && name.trim().toLowerCase() === ownEmail.trim().toLowerCase() ? 'Alex' : name; }
