...
export default function PrivacyPolicy() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.wrap}>
    ...
    <Card><Text style={styles.heading}>Data handled</Text><Text style={styles.body}>FlowOS handles the data needed to provide the service: your Google account identity, including basic profile information, FlowOS account identifier, tasks, events, reminders and other commitments you create, application preferences, and data synchronized with Google Calendar and Google Tasks when you connect Google.</Text></Card>
    ...
    <Card><Text style={styles.heading}>Weather and location</Text><Text style={styles.body}>If you enable weather, FlowOS requests foreground device location while the feature is being used. Depending on the permission selected, this may provide precise or approximate coordinates. The coordinates are sent to Open-Meteo to obtain today's forecast and are not stored by FlowOS as a location history or user profile.</Text></Card>
    ...
    <Card><Text style={styles.heading}>Notifications and diagnostics</Text><Text style={styles.body}>FlowOS may use local device notifications for reminders. Limited diagnostic and notification-log information is stored locally on your device to help diagnose application problems and scheduling issues. These local records are not transmitted as an analytics or advertising profile.</Text></Card>
    <Card><Text style={styles.heading}>Security and retention</Text><Text style={styles.body}>Application data is protected by authentication and row-level access controls. Google OAuth tokens are stored in a private server-side area. FlowOS retains account and application data while needed to provide the service, unless deletion is requested or a longer retention period is required by law. Local diagnostic and notification records remain on the device until cleared.</Text></Card>
