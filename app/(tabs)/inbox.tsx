...
import { Card, palette, showAlert, showConfirm } from '@/components/ui';
...
export default function Inbox() {
  ...
  async function save(item: Commitment, confirm: boolean) {
    ...
    catch (error) { showAlert('Controlla', error instanceof Error ? error.message : 'Salvataggio non riuscito.'); }
  ...
  async function remove(item: Commitment, google: boolean) {
    ...
    catch (error) { showAlert('Controlla', error instanceof Error ? error.message : 'Eliminazione non riuscita.'); }
    finally { setBusyId(null); }
  }
  async function confirmGoogleDelete(item: Commitment) {
    const ok = await showConfirm('Eliminare definitivamente da Google?', `"${item.title}" verrà eliminato da Google ${item.kind === 'event' ? 'Calendar' : 'Tasks'} e da FlowOS. Questa operazione non può essere annullata.`, 'Elimina definitivamente');
    if (ok) void remove(item, true);
  }

  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.wrap}>
    <Text style={styles.eyebrow}>FlowOS</Text>
    <Text style={styles.title}>Controlla</Text>
    <Text style={styles.sub}>Qui trovi gli elementi che FlowOS non è riuscito a interpretare con sufficiente sicurezza. Controlla titolo, tipo, data e durata. Puoi modificare, confermare o eliminare ogni elemento.</Text>
    {items.length ? items.map((item) => {
      ...
      return <Card key={item.id}>
        {editing ? <>
          ...
          <Text style={styles.meta}>Tipo proposto: {item.kind}</Text>
          <Text style={styles.meta}>Data: {item.scheduledAt||item.dueAt?formatCommitmentTime(item,item.scheduledAt??item.dueAt!)!=='Tutto il giorno'?new Date(item.scheduledAt??item.dueAt!).toLocaleString('it-IT'):`${new Date(item.scheduledAt??item.dueAt!).toLocaleDateString('it-IT',{timeZone:'UTC'})} · Tutto il giorno`:'non definita'}</Text>
