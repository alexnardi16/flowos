...
import { Card, EmptyState, palette } from '@/components/ui';
...
export default function Today() {
  ...
  const overdueCount = open.filter((item) => isExpired(item)).length;
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.wrap}>
    ...
    {todayItems.length ? todayItems.map((item)=>{
      ...
      return <Card key={item.id} style={s.itemCard}>
        <View style={s.itemHeader}>
          <View style={{flex:1}}><Text style={s.task}>{item.title}</Text><Text style={s.meta}>{timeLabel} · {formatDurationLabel(item)}{expired?' · scaduta':''}</Text></View>
...
const s=StyleSheet.create({
  ...
  itemCard:{gap:8},itemHeader:{flexDirection:'row',alignItems:'flex-start',gap:8},task:{fontSize:19,lineHeight:24,fontWeight:'900',color:palette.ink},meta:{fontSize:13,lineHeight:18,color:palette.muted},description:{fontSize:14,lineHeight:20,color:palette.ink},
