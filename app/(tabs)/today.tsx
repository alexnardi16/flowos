...
function isToday(item: Commitment) {
  const value = when(item); if (!value) return false;
...
export default function Today() {
  ...
  const todayItems = useMemo(() => open.filter(isToday).filter((item) => item.kind!=='event' || (contactsFilter==='onlyContacts'?isContactEvent(item):contactsFilter==='excludeContacts'?!isContactEvent(item):true)).sort((a,b) => { const av=when(a), bv=when(b); if(!av) return 1; if(!bv) return -1; return new Date(av).getTime()-new Date(bv).getTime(); }), [open,contactsFilter]);
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.wrap}>
    <Text style={s.hello}>FlowOS</Text><Text style={s.title}>Oggi</Text>
    <View style={s.stats}><Card style={s.stat}><Text style={s.statValue}>{todayItems.length}</Text><Text style={s.statLabel}>elementi</Text></Card><Card style={s.stat}><Text style={s.statValue}>{totalLoadLabel(plannedMinutes)}</Text><Text style={s.statLabel}>carico previsto</Text></Card><Pressable onPress={()=>{if(overdueCount) setManageId(todayItems.find((item)=>isExpired(item))?.id??null);}} style={{flex:1}}><Card style={s.stat}><Text style={[s.statValue,overdueCount>0&&s.warning]}>{overdueCount}</Text><Text style={s.statLabel}>attività scadute</Text></Card></Pressable></View>
    ...
    {todayItems.length ? todayItems.map((item)=>{
      ...
      return <Pressable key={item.id} onPress={()=>setManageId(item.id)} style={({pressed})=>pressed&&s.cardPressed}>
        <Card style={[s.itemCard,cardKindStyle(item.kind)]}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.tagRow}>
            <Chip tone={kindTone(item.kind)}>{kindLabel(item.kind)}</Chip><CommitmentSourceTag item={item} google={google}/><Pressable onPress={()=>setManageId(item.id)} style={s.manageButton}><Text style={s.manageButtonText}>Gestisci</Text></Pressable>
          </ScrollView>
          <Text style={s.task}>{item.title}</Text><Text style={s.meta}>{timeLabel} · {formatDurationLabel(item)}{expired?' · scaduta':''}</Text>
          {item.location?<Text style={s.meta}>📍 {item.location}</Text>:null}{item.description?<Text style={s.description}>{item.description}</Text>:null}
          <View style={s.actions}>
            ...
            <Pressable onPress={()=>void postpone(item.id)} style={s.secondaryAction}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.78} style={s.secondaryActionText}>Rimanda di 1 giorno</Text></Pressable>
...
const s=StyleSheet.create({
  ...
  tagRow:{alignItems:'center',gap:7,paddingRight:4},task:{fontSize:19,lineHeight:24,fontWeight:'900',color:palette.ink},meta:{fontSize:13,lineHeight:18,color:palette.muted},description:{fontSize:14,lineHeight:20,color:palette.ink},
  ...
  actions:{flexDirection:'row',gap:6,flexWrap:'nowrap'},secondaryAction:{flex:1,backgroundColor:'#ECEEF4',borderRadius:12,paddingVertical:10,paddingHorizontal:4,alignItems:'center',justifyContent:'center'},secondaryActionText:{fontSize:12,fontWeight:'900',color:palette.ink,textAlign:'center'},primaryAction:{flex:1,backgroundColor:palette.primary,borderRadius:12,paddingVertical:10,alignItems:'center',justifyContent:'center'},primaryActionText:{fontSize:12,fontWeight:'900',color:'#FFF'},cardPressed:{opacity:.96,transform:[{scale:.995}]},
});
