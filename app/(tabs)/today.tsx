...
export default function Today() {
  ...
  const events = useMemo(() => open.filter((item) => item.kind==='event' && when(item) && new Date(when(item)!).getTime()>=Date.now()-3600000).sort((a,b)=>new Date(when(a)!).getTime()-new Date(when(b)!).getTime()), [open]);
  const current = tasks.find((item) => isToday(when(item))) ?? tasks[0];
  const event = events[0];
  const todayItems = open.filter((item) => isToday(when(item)));
