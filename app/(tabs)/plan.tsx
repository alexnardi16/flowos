...
import { Card, Chip, EmptyState, SectionTitle, palette } from '@/components/ui';
...
import { generateAIPlanningSuggestions, type AIPlanningSuggestion } from '@/lib/aiPlanning';
...
function cardKindStyle(kind:Commitment['kind']){return kind==='event'?styles.cardEvent:kind==='task'?styles.cardTask:kind==='reminder'?styles.cardReminder:undefined;}
...
export default function Plan(){
  ...
  const [aiSuggestions,setAiSuggestions]=useState<AIPlanningSuggestion[]>([]);
  ...
  useEffect(()=>{
    let cancelled=false;
    void generateAIPlanningSuggestions(commitments,new Date()).then((result)=>{if(cancelled)return;setAiSuggestions(result.suggestions);setAiSource(result.generatedBy);});
    return ()=>{cancelled=true;};
  },[commitments]);

  const items=useMemo(()=>{const normalized=query.trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();return commitments.filter((item)=>{const event=item.kind==='event';const task=item.kind==='task';const reminder=item.kind==='reminder'||item.kind==='routine';const flexible=task||reminder;const date=itemDate(item);const past=Boolean(date&&new Date(date).getTime()<now);const open=item.status!=='done';if(event&&!filters.events)return false;if(task&&!filters.tasks)return false;if(reminder&&!filters.reminders)return false;if(!event&&!flexible)return false;if(past&&!filters.past&&!(flexible&&open&&filters.overdue))return false;if(item.status==='done'&&!filters.past)return false;if(contactsFilter==='onlyContacts'&&!isContactEvent(item))return false;if(contactsFilter==='excludeContacts'&&isContactEvent(item))return false;return !normalized||searchable(item).includes(normalized);}).sort((a,b)=>{const ad=itemDate(a),bd=itemDate(b);if(!ad)return 1;if(!bd)return-1;return new Date(ad).getTime()-new Date(bd).getTime();});},[commitments,filters,query,contactsFilter,now]);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.wrap}>
    ...
    {syncProgress ? <View style={styles.syncBar}><View style={[styles.syncBarFill,{width:`${syncProgress.percent}%`}]}/><Text style={styles.syncBarText}>{syncProgress.stage} · {syncProgress.percent}%</Text></View> : null}
    ...
    <View style={styles.filters}><Filter label="Eventi" active={filters.events} onPress={()=>toggle('events')}/><Filter label="Task" active={filters.tasks} onPress={()=>toggle('tasks')}/><Filter label="Reminder" active={filters.reminders} onPress={()=>toggle('reminders')}/><Filter label="Passati e completati" active={filters.past} onPress={()=>toggle('past')}/><Filter label="Scaduti aperti" active={filters.overdue} onPress={()=>toggle('overdue')}/></View>
    <View style={styles.filters}><Filter label="Tutti" active={contactsFilter==='all'} onPress={()=>setContactsFilter('all')}/><Filter label="Solo contatti" active={contactsFilter==='onlyContacts'} onPress={()=>setContactsFilter('onlyContacts')}/><Filter label="Escludi contatti" active={contactsFilter==='excludeContacts'} onPress={()=>setContactsFilter('excludeContacts')}/></View>

    {overdueItems.length ? <Card style={styles.overdueCard}>
      <View style={styles.overdueHeader}><Text style={styles.overdueTitle}>Attività in ritardo</Text><Chip tone="warning">{overdueItems.length}</Chip></View>
      {overdueItems.map((item)=><View key={item.id} style={styles.overdueItem}><Text style={styles.overdueItemTitle}>{item.title}</Text><Text style={styles.overdueItemMeta}>{formatDateTime(item)} · {item.kind==='event'?'Evento':item.kind==='reminder'?'Reminder':'Task'}</Text></View>)}
    </Card> : null}

    {aiSuggestions.length?<View style={styles.aiSection}><View style={styles.aiHeader}><Text style={styles.aiTitle}>Pianificazione intelligente</Text><Chip tone={aiSource==='ai'?'success':'primary'}>{aiSource==='ai'?'IA':'Fallback'}</Chip></View>{aiSuggestions.map((suggestion)=><Card key={suggestion.id} style={styles.suggestionCard}><Text style={styles.aiPriority}>{suggestion.priority==='high'?'PRIORITÀ ALTA':suggestion.priority==='low'?'SUGGERIMENTO':'CONSIGLIO'}</Text><Text style={styles.suggestionText}>{suggestion.text}</Text></Card>)}</View>:null}
    ...
    {items.length?items.map((item)=>{const overdue=item.status!=='done'&&isExpired(item);return <Card key={item.id} style={[styles.itemCard,cardKindStyle(item.kind)]}>
      <View style={styles.row}><View style={styles.chips}><Chip tone={item.status==='done'?'success':overdue?'warning':'primary'}>{item.kind==='event'?'EVENTO':item.kind==='reminder'?'REMINDER':item.status==='done'?'COMPLETATA':'TASK'}</Chip><CommitmentSourceTag item={item} google={google}/></View><Pressable onPress={()=>setManageId(item.id)} style={styles.manage}><Text style={styles.manageText}>Gestisci</Text></Pressable></View>
...
const styles=StyleSheet.create({
  safe:{flex:1,backgroundColor:palette.bg},wrap:{padding:20,paddingBottom:110,gap:14},eyebrow:{fontSize:12,fontWeight:'900',letterSpacing:1.4,color:palette.primary,marginTop:14},title:{fontSize:32,fontWeight:'900',color:palette.ink},sub:{fontSize:15,lineHeight:22,color:palette.muted},
  syncBar:{height:28,borderRadius:14,backgroundColor:'#ECEEF4',overflow:'hidden',justifyContent:'center'},syncBarFill:{position:'absolute',left:0,top:0,bottom:0,backgroundColor:palette.soft},syncBarText:{fontSize:11,fontWeight:'800',color:palette.primary,textAlign:'center'},summary:{flexDirection:'row',gap:8,flexWrap:'nowrap'},summaryCard:{flex:1,minWidth:0,padding:10},summaryValue:{fontSize:22,fontWeight:'900',color:palette.ink},summaryLabel:{fontSize:12,color:palette.muted,marginTop:3},warning:{color:palette.warning},
  overdueCard:{backgroundColor:'#FFF7E8',borderColor:'#F3DCA8',borderWidth:1,gap:9},overdueHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8},overdueTitle:{fontSize:16,fontWeight:'900',color:palette.ink},overdueItem:{paddingTop:7,borderTopWidth:1,borderTopColor:'#F3DCA8'},overdueItemTitle:{fontSize:15,fontWeight:'900',color:palette.ink},overdueItemMeta:{fontSize:12,lineHeight:17,color:palette.muted,marginTop:2},
  aiSection:{gap:8},aiHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},aiTitle:{fontSize:16,fontWeight:'900',color:palette.ink},aiPriority:{fontSize:10,fontWeight:'900',color:palette.primary,letterSpacing:.7},suggestionCard:{width:'100%',padding:12,backgroundColor:palette.soft},suggestionText:{fontSize:13,lineHeight:18,color:palette.ink,fontWeight:'700'},
  search:{backgroundColor:'#FFF',borderWidth:1,borderColor:palette.border,borderRadius:15,paddingHorizontal:15,paddingVertical:13,fontSize:15,color:palette.ink},filters:{flexDirection:'row',flexWrap:'wrap',gap:8},filter:{borderRadius:99,paddingHorizontal:12,paddingVertical:9,backgroundColor:'#ECEEF4'},filterActive:{backgroundColor:palette.primary},filterText:{fontSize:12,fontWeight:'800',color:palette.muted},filterTextActive:{color:'#FFF'},
  itemCard:{gap:7},cardEvent:{backgroundColor:'#EEF1FE',borderColor:'#C7D0FB',borderWidth:1},cardTask:{backgroundColor:'#FFF7E8',borderColor:'#F3DCA8',borderWidth:1},cardReminder:{backgroundColor:'#EAFBF3',borderColor:'#B9EAD4',borderWidth:1},row:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8},chips:{flexDirection:'row',gap:7,flexWrap:'wrap',flex:1},manage:{paddingHorizontal:11,paddingVertical:7,borderRadius:10,backgroundColor:palette.soft},manageText:{fontSize:12,fontWeight:'900',color:palette.primary},item:{fontSize:19,fontWeight:'900',color:palette.ink},date:{fontSize:14,fontWeight:'800',color:palette.primary},meta:{fontSize:13,lineHeight:18,color:palette.muted},description:{fontSize:14,lineHeight:20,color:palette.ink,marginTop:2}
});
