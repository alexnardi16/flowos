...
export default function Plan(){
  ...
  function confirmGoogleDelete(item:Commitment){Alert.alert('Eliminare definitivamente da Google?',`“${item.title}” verrà eliminato da Google ${item.kind==='event'?'Calendar':'Tasks'} e da FlowOS. Questa operazione non può essere annullata.`,[{text:'Annulla',style:'cancel'},{text:'Elimina definitivamente',style:'destructive',onPress:()=>void removeGoogle(item.id)}]);}
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.wrap}>
    ...
    {items.length?items.map((item)=>{const actions=openActionsId===item.id;const overdue=item.kind!=='event'&&item.status!=='done'&&item.dueAt&&new Date(item.dueAt).getTime()<Date.now();return <Card key={item.id} style={styles.itemCard}>
      ...
      <Text style={styles.item}>{item.title}</Text><Text style={[styles.date,overdue&&styles.warning]}>{formatDateTime(itemDate(item))}{overdue?' · scaduta':''}</Text>
