...
function formatSyncDate(value?: string | null) {
  if (!value) return 'mai';
  return new Date(value).toLocaleString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' });
}
...
export default function Me() {
  ...
  function clearLogs() { clearDiagnostics(); showAlert('Logger', 'Log ripulito.'); }
  ...
  useEffect(() => {
    const unsubscribe = subscribeToSyncProgress(({ percent, stage }) => {
      ...
      setSyncStage(stage);
      if (percent >= 100) setTimeout(() => { setSyncProgress(0); void loadGoogle(false); }, 900);
    });
    return unsubscribe;
  }, []);
  ...
  async function run(action: () => Promise<unknown>, refreshCommitments = false) {
    ...
    try { await action(); if (refreshCommitments) await hydrateFromCloud(); await loadGoogle(false); }
    catch (error) { const message = error instanceof Error ? error.message : 'Operazione Google non riuscita.'; setGoogleError(message); recordDiagnostic('google-operation-failed', { message }, 'error'); showAlert('Google Workspace', message); await loadGoogle(false); }
    finally { setGoogleBusy(false); }
  }

  async function runSync() {
    if (googleBusy) return;
    setGoogleBusy(true); setGoogleError(null); setSyncProgress(1); setSyncStage('Avvio della sincronizzazione'); recordDiagnostic('sync-ui-started');
    try {
      if (session?.provider_token) { await connectGoogleFromSession(session, true); recordDiagnostic('google-workspace-connection-refreshed-manually', { userId: session.user.id }); }
      await syncGoogleWorkspace(({ percent, stage }) => { setSyncProgress(percent); setSyncStage(stage); });
      await hydrateFromCloud(); await loadGoogle(false); recordDiagnostic('sync-ui-completed');
    } catch (error) { const message = error instanceof Error ? error.message : 'Sincronizzazione non riuscita.'; setGoogleError(message); setSyncStage('Sincronizzazione interrotta'); recordDiagnostic('sync-ui-failed', { message }, 'error'); showAlert('Sincronizzazione Google', message); await loadGoogle(false); }
    finally { setGoogleBusy(false); setTimeout(() => { setSyncProgress(0); setSyncStage(''); }, 1800); }
  }

  async function logout() {
    if (googleBusy) return;
    try { await signOut(); router.replace('/login'); }
    catch (error) { const message = error instanceof Error ? error.message : 'Logout non riuscito.'; recordDiagnostic('flowos-logout-failed', { message }, 'error'); showAlert('FlowOS', message); }
  }
  ...
  async function wipeFlowOSData() {
    ...
    if (!ok) return;
    setGoogleBusy(true);
    try { await deleteAllFlowOSOnlyData(); await hydrateFromCloud(); recordDiagnostic('flowos-data-wiped'); showAlert('FlowOS', 'Tutti gli elementi FlowOS sono stati eliminati.'); }
    catch (error) { const message = error instanceof Error ? error.message : 'Eliminazione non riuscita.'; recordDiagnostic('flowos-data-wipe-failed', { message }, 'error'); showAlert('FlowOS', message); }
    finally { setGoogleBusy(false); }
  }

  async function deleteAccount() {
    if (googleBusy) return;
    const ok = await showConfirm('Eliminare definitivamente l\'account?', 'Questa operazione elimina irreversibilmente i dati FlowOS, i token Google e l\'account di autenticazione. L\'operazione non può essere annullata.', 'Elimina definitivamente');
    if (!ok) return;
    setGoogleBusy(true);
    try { await deleteFlowOSAccount(); recordDiagnostic('flowos-account-deleted'); router.replace('/login'); }
    catch (error) { const message = error instanceof Error ? error.message : 'Eliminazione dell\'account non riuscita.'; recordDiagnostic('flowos-account-delete-failed', { message }, 'error'); showAlert('FlowOS', message); }
    finally { setGoogleBusy(false); }
  }
  ...
  useEffect(()=>{ if(!google?.range)return; const toDisplay=(label:string)=>{const [d,m,y]=label.split('/');return d&&m&&y?`${d}-${m}-${y}`:'';}; setRangeStartInput(toDisplay(google.range.labelStart)); setRangeEndInput(toDisplay(google.range.labelEnd)); },[google?.range?.labelStart,google?.range?.labelEnd]);
  async function saveRange(){ const toIso=(display:string)=>{const [d,m,y]=display.split('-');return d&&m&&y?`${y}-${m}-${d}`:null;}; const startIso=toIso(rangeStartInput), endIso=toIso(rangeEndInput); if(!startIso||!endIso||!/^\d{4}-\d{2}-\d{2}$/.test(startIso)||!/^\d{4}-\d{2}-\d{2}$/.test(endIso)){showAlert('Google Workspace','Usa il formato GG-MM-AAAA per entrambe le date.');return;} setRangeSaving(true); try{await setSyncRange(startIso,endIso);await loadGoogle(false);showAlert('Google Workspace','Periodo aggiornato. Sincronizza ora per applicarlo.');}catch(error){showAlert('Google Workspace',error instanceof Error?error.message:'Aggiornamento non riuscito.');}finally{setRangeSaving(false);} }
  const rangeLabel = google?.range ? `Elementi importati visibili dal ${google.range.labelStart} al ${google.range.labelEnd}.` : 'Intervallo di importazione in caricamento…';
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.wrap}>
    <Text style={styles.eyebrow}>FlowOS</Text><Text style={styles.title}>Impostazioni</Text>
    <Card><Text style={styles.label}>Google Workspace</Text><Text style={styles.rangeLabel}>{rangeLabel}</Text><View style={styles.inline}><View style={styles.flex}><Text style={styles.fieldLabel}>Dal</Text><TextInput value={rangeStartInput} onChangeText={setRangeStartInput} placeholder="GG-MM-AAAA" style={styles.rangeInput}/></View><View style={styles.flex}><Text style={styles.fieldLabel}>Al</Text><TextInput value={rangeEndInput} onChangeText={setRangeEndInput} placeholder="GG-MM-AAAA" style={styles.rangeInput}/></View></View><View style={styles.actions}><Button secondary label={rangeSaving?'Salvataggio…':'Salva periodo'} onPress={()=>{void saveRange();}} disabled={rangeSaving}/></View>
      {googleConnected ? <><Text style={styles.item}>{google?.connection?.google_email ? friendlyCalendarName(google.connection.google_email, google.connection.google_email) : 'Account Google collegato'}</Text><Text style={styles.meta}>Stato: {google?.connection?.last_sync_status} · Ultima sincronizzazione: {formatSyncDate(google?.connection?.last_sync_at)}</Text>{google?.connection?.last_sync_error ? <Text style={styles.error}>{google.connection.last_sync_error}</Text> : null}{googleError && googleError !== google?.connection?.last_sync_error ? <Text style={styles.error}>{googleError}</Text> : null}{syncProgress > 0 ? <View style={styles.progressBlock}><View style={styles.progressHeader}><Text style={styles.progressStage}>{syncStage}</Text><Text style={styles.progressPercent}>{syncProgress}%</Text></View><View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${syncProgress}%` }]} /></View></View> : null}<View style={styles.actions}><Button label={googleBusy ? 'Sincronizzazione…' : 'Sincronizza ora'} onPress={() => { void runSync(); }}/><Button secondary label="Scollega Google" onPress={() => { void run(disconnectGoogleWorkspace); }}/></View><View style={styles.wipeRow}><Pressable onPress={() => { void wipeFlowOSData(); }} style={styles.wipeButton}><Text style={styles.logoutText}>Elimina ogni attività su FlowOS senza alcun impatto su Google</Text></Pressable></View></> : <><Text style={styles.meta}>{authorizationMissing ? 'La sessione FlowOS è attiva, ma l’autorizzazione Google deve essere ripristinata.' : 'Collega Google per sincronizzare Calendar e Tasks in entrambe le direzioni.'}</Text>{googleError ? <Text style={styles.error}>{googleError}</Text> : null}<View style={styles.actions}><Button label={authorizationMissing ? 'Ricollega Google' : 'Collega Google'} onPress={() => { void run(signInWithGoogle); }}/></View></>}
    </Card>
    {googleConnected ? <><Card><Text style={styles.label}>Calendari sincronizzati</Text><Text style={styles.meta}>Puoi includere più calendari, anche condivisi. Solo quelli con permesso di scrittura possono essere usati per creare o modificare eventi.</Text>{google?.calendars.map((calendar) => {const writable=['owner','writer'].includes(calendar.access_role);const holidayCalendar=/^Jours fériés en (France|Italie)$/i.test(calendar.summary.trim());return <View key={calendar.id} style={styles.resourceRow}><View style={styles.resourceText}><Text style={styles.resourceTitle}>{friendlyCalendarName(calendar.summary,google?.connection?.google_email)}{calendar.primary_calendar&&friendlyCalendarName(calendar.summary,google?.connection?.google_email)!=='Alex'?' · principale':''}</Text><Text style={styles.meta}>{calendar.access_role}{calendar.is_default?' · predefinito':''}</Text></View><Switch value={calendar.selected} onValueChange={(selected)=>{void run(()=>setCalendarSelected(calendar.id,selected));}}/>{!holidayCalendar?<Pressable disabled={!calendar.selected||!writable||calendar.is_default} onPress={()=>{void run(()=>setDefaultCalendar(calendar.id));}} style={[styles.defaultButton,(!calendar.selected||!writable||calendar.is_default)&&styles.disabled]}><Text style={styles.defaultText}>{calendar.is_default?'Default':'Imposta come default'}</Text></Pressable>:null}</View>;})}</Card><Card><Text style={styles.label}>Liste Google Tasks</Text><Text style={styles.meta}>La lista predefinita viene proposta automaticamente, ma puoi cambiarla per ogni nuova attività.</Text>{google?.taskLists.map((list)=><View key={list.id} style={styles.resourceRow}><View style={styles.resourceText}><Text style={styles.resourceTitle}>{list.title}</Text><Text style={styles.meta}>{list.is_default?'Predefinita':'Lista attività'}</Text></View><Switch value={list.selected} onValueChange={(selected)=>{void run(()=>setTaskListSelected(list.id,selected));}}/><Pressable disabled={!list.selected||list.is_default} onPress={()=>{void run(()=>setDefaultTaskList(list.id));}} style={[styles.defaultButton,(!list.selected||list.is_default)&&styles.disabled]}><Text style={styles.defaultText}>{list.is_default?'Default':'Imposta come default'}</Text></Pressable></View>)}</Card></>:null}
