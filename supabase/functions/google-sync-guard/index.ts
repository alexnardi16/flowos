...
async function resolve(userId:string,id:string,resolution:string){const {data:conflict,error}=await admin.from('sync_conflicts').select('*').eq('id',id).eq('user_id',userId).eq('status','open').maybeSingle();if(error)throw error;if(!conflict)throw new Error('Conflitto non trovato');const itemId=conflict.commitment_id;const localWasDeleted=Boolean(conflict.local_snapshot?.deleted_at);if(conflict.conflict_type==='remote_deleted'){if(itemId)await admin.from('commitments').delete().eq('id',itemId).eq('user_id',userId);}else if(resolution==='keep_google'){if(itemId)await admin.from('commitments').delete().eq('id',itemId).eq('user_id',userId);}else if(resolution==='keep_flowos'){if(itemId){const update:any={sync_status:'pending',sync_error:null};if(localWasDeleted)update.deleted_at=conflict.local_snapshot.deleted_at;await admin.from('commitments').update(update).eq('id',itemId).eq('user_id',userId);}}else if(resolution==='keep_both'){if(itemId)await admin.from('commitments').update({external_provider:null,external_resource_type:null,external_id:null,external_etag:null,external_updated_at:null,google_calendar_id:null,google_task_list_id:null,last_sync_origin:'flowos',sync_status:'local_only',sync_error:null}).eq('id',itemId).eq('user_id',userId);}else throw new Error('Risoluzione non supportata');await admin.from('sync_conflicts').update({status:'resolved',resolution,resolved_at:new Date().toISOString()}).eq('id',id).eq('user_id',userId);return{ok:true};}
